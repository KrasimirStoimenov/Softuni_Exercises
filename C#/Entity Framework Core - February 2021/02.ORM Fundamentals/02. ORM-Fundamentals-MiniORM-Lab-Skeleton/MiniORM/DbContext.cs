    ...
    public abstract class DbContext
    {
        ...
        private string[] GetEntityColumnNames(Type table)
        {
            ...
            var columns = table.GetProperties()
                .Where(x => dbColumns.Contains(x.Name) && !x.HasAttribute<NotMappedAttribute>() && AllowedSqlTypes.Contains(x.PropertyType))
                .Select(x => x.Name)
                .ToArray();
        ...
        {
            var tableName = ((TableAttribute)Attribute.GetCustomAttribute(table, typeof(TableAttribute)))?.Name;
            if (tableName == null)
            {
